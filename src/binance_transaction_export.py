...
class BinanceTransactionExport:
    def __init__(self):
        ...
        self.pairs = {}
        for record in requests.get("https://api.binance.com/api/v1/exchangeInfo", timeout=1000).json()["symbols"]:
    ...
    def read_export(self, df):
        ...
        while i < len(df.index):
            ...
            buy = self.__get_value_and_pair(record[const.BINANCE_TRANSACTION_EXECUTED_COLUMM], record[const.BINANCE_TRANSACTION_PAIR_COLUMM])
            price = self.__get_value_and_pair(record[const.BINANCE_TRANSACTION_AMOUNT_COLUMM], record[const.BINANCE_TRANSACTION_PAIR_COLUMM])
            fee = self.__get_value_and_pair(record[const.BINANCE_TRANSACTION_FEE_COLUMM], record[const.BINANCE_TRANSACTION_PAIR_COLUMM])
